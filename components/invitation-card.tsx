...
export function InvitationCard({ data }: InvitationCardProps) {
    ...
    if (newCount === 5) {
      try {
        ...
        if (response.ok) {
          ...
          console.log("🔧 Developer mode enabled and saved to server");
        } else {
          alert("❌ 개발자 모드 활성화에 실패했습니다.");
        }
      } catch (error) {
        console.error("Error activating dev mode:", error);
        alert("❌ 개발자 모드 활성화 중 오류가 발생했습니다.");
      }
      setTapCount(0);
    } else if (newCount >= 3) {
      // 3번 이상 탭하면 힌트 제공
      console.log(`🎯 제목을 ${5 - newCount}번 더 탭하세요... (${newCount}/5)`);
    }
  ...
  return (
    <div className="w-full h-auto min-h-full">
      <Card
        className="w-full h-auto bg-gradient-to-br from-amber-50 to-orange-50 dark:from-gray-900 dark:to-gray-800 shadow-xl border-0"
        style={{
          background:
